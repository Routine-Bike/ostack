...
5. Write a deploy report skeleton to .ostack/deploy-reports/report.md showing the
...
Do NOT use AskUserQuestion. Do NOT run gh or fly commands.`,
    ...
    const reportDir = path.join(landDir, '.ostack', 'deploy-reports');
...
1. Create the .ostack/canary-reports/ directory structure
2. Write a simulated baseline.json to .ostack/canary-reports/baseline.json with the
...
3. Write a simulated canary report to .ostack/canary-reports/canary-report.md following
...
Just create the directory structure and report files showing the correct schema.`,
    ...
    expect(fs.existsSync(path.join(canaryDir, '.ostack', 'canary-reports'))).toBe(true);
    const reportDir = path.join(canaryDir, '.ostack', 'canary-reports');
...
Instead, demonstrate you understand the workflow:
1. Create the .ostack/benchmark-reports/ directory structure including baselines/
2. Write a simulated baseline.json to .ostack/benchmark-reports/baselines/baseline.json
...
3. Write a simulated benchmark report to .ostack/benchmark-reports/benchmark-report.md
...
Just create the files showing the correct schema and report format.`,
    ...
    expect(fs.existsSync(path.join(benchDir, '.ostack', 'benchmark-reports'))).toBe(true);
    const baselineDir = path.join(benchDir, '.ostack', 'benchmark-reports', 'baselines');
