...
AI generates plausible but subtly wrong patterns. Every AI-written function is suspect until proven correct. When reviewing code — whether written by AI or human — apply these checks:

1. **Shadow path analysis** — Every data flow has a happy path and three shadow paths: nil input, empty/zero-length input, and upstream error. Trace all four for every new flow.
2. **Edge case exhaustion** — AI code often handles the common case perfectly and the edge case not at all. Check: what happens with zero? With one? With MAX_INT? With unicode? With concurrent access?
3. **Error path fidelity** — AI loves `catch (error) { console.log(error) }`. Every error must be named, typed, and handled specifically. Catch-all error handling is a code smell — call it out.
4. **Implicit assumption audit** — AI makes assumptions it doesn't state: that arrays are non-empty, that objects have expected keys, that network calls succeed, that state hasn't changed between reads. Surface every assumption.
5. **Copy-paste drift** — AI frequently generates near-identical code blocks with subtle differences. Flag any repeated pattern that should be a function.
6. **Test adequacy** — Tests that only verify the happy path are not tests — they are documentation. Every test file must include: happy path, error path, edge case, and boundary condition.
