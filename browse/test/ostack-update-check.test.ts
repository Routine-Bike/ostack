/**
 * Tests for bin/ostack-update-check bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * OSTACK_DIR / OSTACK_STATE_DIR / OSTACK_REMOTE_URL env overrides
 * for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'ostack-update-check');

let ostackDir: string;
let stateDir: string;
...
function run(extraEnv: Record<string, string> = {}, args: string[] = []) {
  const result = Bun.spawnSync(['bash', SCRIPT, ...args], {
    env: {
      ...process.env,
      OSTACK_DIR: ostackDir,
      OSTACK_STATE_DIR: stateDir,
      OSTACK_REMOTE_URL: `file://${join(ostackDir, 'REMOTE_VERSION')}`,
      ...extraEnv,
    },
    stdout: 'pipe',
    stderr: 'pipe',
...
beforeEach(() => {
  ostackDir = mkdtempSync(join(tmpdir(), 'ostack-upd-test-'));
  stateDir = mkdtempSync(join(tmpdir(), 'ostack-state-test-'));
  // Link real ostack-config so update_check config check works
  const binDir = join(ostackDir, 'bin');
  mkdirSync(binDir);
  symlinkSync(join(import.meta.dir, '..', '..', 'bin', 'ostack-config'), join(binDir, 'ostack-config'));
});

afterEach(() => {
  rmSync(ostackDir, { recursive: true, force: true });
...
describe('ostack-update-check', () => {
  ...
  test('exits 0 with no output when VERSION file is empty', () => {
    writeFileSync(join(ostackDir, 'VERSION'), '');
    const { exitCode, stdout } = run();
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
  });
  ...
  test('outputs JUST_UPGRADED and deletes marker', () => {
    writeFileSync(join(ostackDir, 'VERSION'), '0.4.0\n');
  ...
  test('exits silently when cache says UP_TO_DATE and is fresh', () => {
    writeFileSync(join(ostackDir, 'VERSION'), '0.3.3\n');
  ...
  test('re-checks when UP_TO_DATE cache version does not match local', () => {
    ...
    writeFileSync(join(ostackDir, 'REMOTE_VERSION'), '0.5.0\n');
  ...
  test('re-checks when local version does not match cached old version', () => {
    ...
    writeFileSync(join(ostackDir, 'REMOTE_VERSION'), '0.4.0\n');
  ...
  test('writes UP_TO_DATE cache when versions match', () => {
    writeFileSync(join(ostackDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(ostackDir, 'REMOTE_VERSION'), '0.3.3\n');
  ...
  test('treats invalid remote response as up to date', () => {
    writeFileSync(join(ostackDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(ostackDir, 'REMOTE_VERSION'), '<html>404 Not Found</html>\n');
  ...
  test('exits silently when remote URL is unreachable', () => {
    ...
    const { exitCode, stdout } = run({
      OSTACK_REMOTE_URL: 'file:///nonexistent/path/VERSION',
    });
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
    const cache = readFileSync(join(stateDir, 'last-update-check'), 'utf-8');
  ...
  test('creates state dir if it does not exist', () => {
    ...
    const { exitCode } = run({ OSTACK_STATE_DIR: newStateDir });
    expect(exitCode).toBe(0);
  ...
  test('exits 0 with real project VERSION and unreachable remote', () => {
    ...
    writeFileSync(join(ostackDir, 'VERSION'), version + '\n');
