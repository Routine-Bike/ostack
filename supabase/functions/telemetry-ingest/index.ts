// ostack telemetry-ingest edge function
// Validates and inserts a batch of telemetry events.
// Called by bin/ostack-telemetry-sync.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const event of events) {
      // Required fields
      if (!event.ts || !event.ostack_version || !event.os || !event.outcome) {
      ...
      rows.push({
        ...
        ostack_version: String(event.ostack_version).slice(0, 20),
