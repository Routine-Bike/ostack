/**
 * Tests for bin/ostack-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * OSTACK_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'ostack-config');
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'ostack-config-test-'));
});
...
describe('ostack-config', () => {
  ...
  test('set creates state dir if missing', () => {
    ...
    const { exitCode } = run(['set', 'foo', 'bar'], { OSTACK_STATE_DIR: nestedDir });
    expect(exitCode).toBe(0);
