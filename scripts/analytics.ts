#!/usr/bin/env bun
/**
 * analytics — CLI for viewing ostack skill usage statistics.
 *
 * Reads ~/.ostack/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.ostack', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('ostack skill usage analytics');
