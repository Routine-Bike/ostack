...
> **ostack** is a fork of [gstack](https://github.com/garrytan/gstack) by [Garry Tan](https://x.com/garrytan).
>
> Massive thanks to Garry for open-sourcing this incredible project. gstack is an AI-powered
> engineering team that turns Claude Code into a virtual software factory — and Garry made it
> free and MIT-licensed for everyone. This fork builds on that foundation to create a
> customized version for my own workflow. All credit for the original architecture, skills,
> and philosophy goes to Garry and the gstack contributors.

ostack turns Claude Code into a virtual engineering team — a CEO who rethinks the product, an eng manager who locks architecture, a designer who catches AI slop, a reviewer who finds production bugs, a QA lead who opens a real browser, a security officer who runs OWASP + STRIDE audits, and a release engineer who ships the PR. Twenty specialists and eight power tools, all slash commands, all Markdown, all free, MIT license.
...
1. Install ostack (30 seconds — see below)
...
ostack works on any agent that supports the [SKILL.md standard](https://github.com/anthropics/claude-code). Skills live in `.agents/skills/` and are discovered automatically.
...
When setup runs from `.agents/skills/ostack`, it installs the generated Codex skills next to it in the same repo and does not write to `~/.codex/skills`.
...
`setup --host codex` creates the runtime root at `~/.codex/skills/ostack` and
...
For Codex-compatible hosts, setup now supports both repo-local installs from `.agents/skills/ostack` and user-global installs from `~/.codex/skills/ostack`. All 28 skills work across all supported agents. Hook-based safety skills (careful, freeze, guard) use inline safety advisory prose on non-Claude hosts.
...
| `/ostack-upgrade` | **Self-Updater** — upgrade ostack to latest. Detects global vs vendored install, syncs both, shows what changed. |
...
Free, MIT licensed, open source. Forked from [gstack](https://github.com/garrytan/gstack) by [Garry Tan](https://x.com/garrytan).
...
ostack includes **opt-in** usage telemetry to help improve the project. Here's exactly what happens:

- **Default is off.** Nothing is sent anywhere unless you explicitly say yes.
- **On first run,** ostack asks if you want to share anonymous usage data. You can say no.
- **What's sent (if you opt in):** skill name, duration, success/fail, ostack version, OS. That's it.
- **What's never sent:** code, file paths, repo names, branch names, prompts, or any user-generated content.
- **Change anytime:** `ostack-config set telemetry off` disables everything instantly.
...
**Local analytics are always available.** Run `ostack-analytics` to see your personal usage dashboard from the local JSONL file — no remote data needed.
...
**Windows users:** ostack works on Windows 11 via Git Bash or WSL. Node.js is required in addition to Bun — Bun has a known bug with Playwright's pipe transport on Windows ([bun#4253](https://github.com/oven-sh/bun/issues/4253)). The browse server automatically falls back to Node.js. Make sure both `bun` and `node` are on your PATH.

**Claude says it can't see the skills?** Make sure your project's `CLAUDE.md` has a ostack section. Add this:

```
## ostack
Use /browse from ostack for all web browsing. Never use mcp__claude-in-chrome__* tools.
Available skills: /office-hours, /plan-ceo-review, /plan-eng-review, /plan-design-review,
/design-consultation, /review, /ship, /land-and-deploy, /canary, /benchmark, /browse,
/qa, /qa-only, /design-review, /setup-browser-cookies, /setup-deploy, /retro,
/investigate, /document-release, /codex, /cso, /autoplan, /careful, /freeze, /guard,
/unfreeze, /ostack-upgrade.
```

## License

MIT. Forked from [gstack](https://github.com/garrytan/gstack) by Garry Tan. Go build something.
