...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/ostack-cli-');
    const execPath = path.join(root, '.claude/skills/ostack/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/ostack/browse/src/server.ts');
